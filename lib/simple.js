...
END:VCARD
     ...
     * @param {Object} options
     */
    conn.sendButtonVid= async (jid, path, content, footer, button1, row1, quoted, options = {}) {
      let type = await this.getFile(path)
      let { res, data: file } = type
      if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
    ...
    conn.send2ButtonVid= async (jid, path, content, footer, button1, row1, button2, row2, quoted, options = {}) {
      let type = await this.getFile(path)
      let { res, data: file } = type
      if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
    ...
    conn.send3ButtonVid= async (jid, path, content, footer, button1, row1, button2, row2, button3, row3, quoted, options = {}) {
      let type = await this.getFile(path)
      let { res, data: file } = type
      if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
